...
public class GetVideo : MonoBehaviour
{
	...
	{
		bool granted = NativeGallery.CheckPermission(
		...
		if (granted)
		{
			Debug.Log("Gallery permission granted");
		}
		else
		{
			Debug.LogWarning("Gallery permission denied - will save to app folder only");
		}
	}	
	#endif
	...
	void ReceiveLoop()
	{
		...
		while (isConnected)
		{
			try
			{
				...
				if (packet.Length < 8) continue; // header is 8 bytes

				// Parse header (matches C struct: frame_id u32, chunk_index u16, total_chunks u16)
				uint frameId = (uint)System.Net.IPAddress.NetworkToHostOrder(
					(int)BitConverter.ToUInt32(packet, 0));
				ushort chunkIndex = (ushort)System.Net.IPAddress.NetworkToHostOrder(
					(short)BitConverter.ToUInt16(packet, 4));
				ushort totalChunks = (ushort)System.Net.IPAddress.NetworkToHostOrder(
				...
				if (frameChunks[frameId].Count == totalChunks)
				{
					...
					// Clean up old incomplete frames to avoid memory leak
					var oldFrames = new List<uint>();
					foreach (var k in frameChunks.Keys)
						if (k < frameId - 10) oldFrames.Add(k);
					foreach (var k in oldFrames) { frameChunks.Remove(k); frameTotalChunks.Remove(k); }

					lock (queueLock)
					{
						if (queue.Count > 3) queue.Clear();
						queue.Enqueue(jpeg);
					}
				}
			}
	...
	{
		if (inputTexture == null || inputTexture.width <= 2)
		{
			Debug.LogWarning("No frame to save yet!");
			return;
		}
	
		StartCoroutine(SaveFrameToGallery());
	}

	private IEnumerator SaveFrameToGallery()
	{
		// Get the texture from the material
		Texture2D textureToSave = inputTexture;
	
		// Encode to PNG (better quality than JPEG)
		byte[] pngData = null;
		string tempPath = null;
	
		try
		{
			pngData = textureToSave.EncodeToPNG();
		
			// Create filename with timestamp
			string timestamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
			string filename = $"StreamCapture_{gameObject.name}_{timestamp}_{savedFrameCount}.png";
		
			// Save to persistent data path first
			tempPath = Path.Combine(Application.persistentDataPath, filename);
			File.WriteAllBytes(tempPath, pngData);
		
			Debug.Log($"Saved frame to: {tempPath}");
		}
			catch (Exception e)
			{
				Debug.LogError($"Failed to save frame: {e.Message}");
				yield break;
			}
	
			// Wait a frame for file to be written (OUTSIDE try-catch)
		yield return new WaitForEndOfFrame();
	
		// Save to Meta Quest gallery using NativeGallery plugin
	#if UNITY_ANDROID
		...
		if (!hasPermission)
		{
			Debug.LogWarning("Gallery permission denied. File saved to: " + tempPath);
			yield break;
		}
	
		NativeGallery.SaveImageToGallery(
			tempPath, 
			"StreamCaptures",  // Album name
			...
			(success, path) => {
				...
				{
					Debug.LogError("Failed to save to gallery");
				}
			}
		);
	#else
		Debug.Log("Gallery save only works on Android/Quest. File saved to: " + tempPath);
	#endif
	
		savedFrameCount++;
	}
	public void SaveFrameSimple()
	{
		if (inputTexture == null || inputTexture.width <= 2)
		{
			Debug.LogWarning("No frame to save yet!");
		...
		try
		{
			byte[] pngData = inputTexture.EncodeToPNG();
			string timestamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
			string filename = $"Capture_{timestamp}.png";
			
			// On Quest, this path is accessible via file browser
			string path = Path.Combine(Application.persistentDataPath, filename);
			File.WriteAllBytes(path, pngData);
			
			Debug.Log($"Frame saved to: {path}");
			Debug.Log("Access via: Android/data/com.YourCompany.YourApp/files/");
